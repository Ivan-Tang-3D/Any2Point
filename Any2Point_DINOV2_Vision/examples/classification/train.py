...
def main(gpu, cfg, profile=False, args=None):
    ...
    if args.pretrained_path is not None:
        if args.mode == 'resume': #cfg
            ...
            if args.mode == 'test':
                ...
                # validation mode
                epoch, best_val = load_checkpoint(model, args.pretrained_path)
                macc, oa, accs, cm = validate_fn(model, val_loader, cfg, args)
                print_cls_results(oa, macc, accs, epoch, cfg)
                return True
            elif args.mode == 'finetune':
                # finetune the whole model
                logging.info(f'Finetuning from {args.pretrained_path}')
                load_checkpoint(model, args.pretrained_path)
            elif args.mode == 'finetune_encoder':
                # finetune the whole model
                logging.info(f'Finetuning from {args.pretrained_path}')
                load_checkpoint(model.encoder, args.pretrained_path)
    else:
    ...
    for name, param in model.named_parameters():
        if 'prediction.head' in name or 'encoder.pos_embed' in name or 'encoder.patch_embed' in name or 'encoder.cls_pos' in name or 'encoder.cls_token' in name or 'encoder.norm' in name or 'adapter' in name or 'out_transform' in name or 'attn1' in name or 'norm3' in name or 'attn_3d' in name:
    ...
    best_epoch, _ = load_checkpoint(model, pretrained_path=os.path.join(
        cfg.ckpt_dir, f'{cfg.run_name}_ckpt_best.pth'),if_best=True)
    test_macc, test_oa, test_accs, test_cm = validate(model, test_loader, cfg, args)
    if writer is not None:
...
def train_one_epoch(model, train_loader, optimizer, scheduler, epoch, cfg, args):
    ...
    for idx, data in pbar:
        ...
        logits, loss = model.get_logits_loss(data, target, args=args) if not hasattr(model, 'module') else model.module.get_logits_loss(data, target)
        loss.backward()
        # optimize
...
def validate(model, val_loader, cfg, args):
    ...
    for idx, data in pbar:
        ...
        data['pos'] = points[:, :, :3].contiguous()
        data['x'] = points[:, :, :cfg.model.in_channels].transpose(1, 2).contiguous()
        logits = model(data, args=args, target=target)
        cm.update(logits.argmax(dim=1), target)
