...
class LGA(nn.Module):
    def __init__(self, out_dim, alpha, beta, block_num, dim_expansion, type,adapter_layer=0, lastdim=0):
        super().__init__()
        self.type = type
        ...
        for i in range(block_num):
            self.linear2.append(Linear2Layer(out_dim, bias=True, adapter_layer=self.adapter_layer,lastdim=lastdim))
...
class Linear2Layer(nn.Module):
    def __init__(self, in_channels, kernel_size=1, groups=1, bias=True, adapter_layer=0, lastdim=0):
        super(Linear2Layer, self).__init__()
        self.act = nn.ReLU(inplace=True)
        if adapter_layer==2:
            self.net1 = nn.Sequential(
                nn.Conv2d(in_channels=in_channels, out_channels=int(lastdim),
                        kernel_size=kernel_size, groups=groups, bias=bias),
                nn.BatchNorm2d(int(lastdim)),
                self.act
            )
            self.net2 = nn.Sequential(
                nn.Conv2d(in_channels=int(lastdim), out_channels=in_channels,
...
class EncP(nn.Module):  
    def __init__(self, in_channels, input_points, num_stages, embed_dim, k_neighbors, alpha, beta, LGA_block, dim_expansion, type, lastdim):
        super().__init__()
        ...
        for i in range(self.num_stages):
            ...
            self.LGA_list.append(LGA(out_dim, self.alpha, self.beta, LGA_block[i], dim_expansion[i], type, adapter_layer=i, lastdim=lastdim))
...
class Point_PN_scan(nn.Module):
    def __init__(self, in_channels=3, class_num=15, input_points=1024, num_stages=3, embed_dim=96, beta=100, alpha=1000, LGA_block=[2,1,1,1], dim_expansion=[2,2,2,1], type='scan', k_neighbors=64, lastdim=32):
        super().__init__()
        # Parametric Encoder
        self.EncP = EncP(in_channels, input_points, num_stages, embed_dim, k_neighbors, alpha, beta, LGA_block, dim_expansion, type, lastdim)
