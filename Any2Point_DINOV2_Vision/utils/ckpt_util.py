...
def load_checkpoint(model, pretrained_path, module=None, if_best=False):
    if not os.path.exists(pretrained_path):
    ...
    base_ckpt = {k.replace("module.", ""): v for k, v in ckpt_state_dict.items()} #.replace("cls_token",'origin_cls_token')
