...
def points2pos(points, image_height, image_width, size_x=4, size_y=4, args=None):
    """
    :param points: [B, num_points, 3]
    :param image_width:
    :param image_height:
    ...
    epsilon = torch.tensor([1e-12], requires_grad=False, device=points.device)
    # epsilon not needed, kept here to ensure exact replication of old version
    coord_x = (points[:, :, 0] / (points[:, :, 2] + epsilon))   # [batch, num_points]
    coord_y = (points[:, :, 1] / (points[:, :, 2] + epsilon))* (image_height / image_width)  # [batch, num_points]
    ...
    _x = ((coord_x + 1) * image_width) / 2
    _y = ((coord_y + 1) * image_height) / 2
    ...
    extended_x = (extended_x % image_width)
    extended_y = (extended_y % image_height)
