...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # build model
    config.model.patchknn = args.patchknn
    config.model.adapter_dim = args.adapter_dim
    config.model.trans = args.trans
    ...
    for name, param in base_model.named_parameters():
        if 'cls_head_' in name or 'patch_embed' in name or 'cls_pos' in name or 'cls_token' in name or "norm." in name or ".adapter" in name or "out_transform" in name or "norm3" in name or "attn1." in name: 
...
def test(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            logits = base_model(points,args, label=label)
            target = label.view(-1)
