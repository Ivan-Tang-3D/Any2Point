...
class PointTransformer(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.patch_tokens_2d = 1370 ##including the cls token
    ...
    def forward(self, pts, args, label=None):
        sampled_indices=[]
        for i in range(pts.shape[0]):
            sampled_indices.append(fps(pts[i], ratio=5/1024))
        sampled_indices = torch.stack(sampled_indices,dim=0)

        pos_x,pos_y,aug_points  = self.get_pos(pts, args)
        x_y = torch.stack([pos_x, pos_y], dim=-1)

        x_y = x_y.reshape(pts.shape[0],-1,pts.shape[1],2)
        aug_points = aug_points.reshape(pts.shape[0],-1,pts.shape[1],3)
        ...
        interpolated_pos_embed = interpolated_pos_embed.reshape(pts.shape[0],-1,pts.shape[1],self.trans_dim)
        ...
        pos_x,pos_y,aug_points = self.get_pos(center_p, args)
        x_y = torch.stack([pos_x, pos_y], dim=-1)
        ...
        for _, block in enumerate(self.blocks):
            x = block(x+pos, center1=x_y_2d, idx_ptr=idx_ptr, sorted_cluster_indices=sorted_cluster_indices, cluster=cluster,grid_shape=grid_shape, mask=mask_list, flat_grid_index=flat_grid_index_list,attn1=self.attn1, norm3=self.norm3, args=args)
        x = self.norm(x)
