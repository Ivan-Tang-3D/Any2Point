...
from utils import set_random_seed, save_checkpoint, load_checkpoint, resume_checkpoint, setup_logger_dist, \
    cal_model_parm_nums, Wandb
from utils import AverageMeter, ConfusionMatrix, get_mious
from dataset import build_dataloader_from_cfg
from optim import build_optimizer_from_cfg
from scheduler import build_scheduler_from_cfg
from models import build_model_from_cfg
from models.layers import furthest_point_sample, fps
...
def main(gpu, cfg, profile=False, args=None):
    if cfg.distributed:
        if cfg.mp:
            cfg.rank = gpu
    ...
    if cfg.pretrained_path is not None:
        if cfg.mode == 'resume':
            ...
            macc, oa, accs, cm = validate_fn(model, val_loader, cfg, args)
            print_cls_results(oa, macc, accs, cfg.start_epoch, cfg)
            last_ckpt_path = cfg.pretrained_path
            best_ckpt_path = last_ckpt_path.replace('latest','best')
            best_ckpt = torch.load(best_ckpt_path,map_location='cuda')
            best_epoch, best_val = best_ckpt['epoch'], best_ckpt['best_val']
        else:
            if cfg.mode == 'test':
                # test mode
                epoch, best_val = load_checkpoint(
                    model, pretrained_path=cfg.pretrained_path, if_stat=True)
                macc, oa, accs, cm = validate_fn(model, test_loader, cfg, args)
    ...
    for name, param in model.named_parameters():
        if 'prediction.head' in name or 'encoder.pos_embed' in name or 'encoder.patch_embed' in name or 'encoder.cls_pos' in name or 'encoder.cls_token' in name or 'encoder.norm' in name or 'ls' in name or 'adapter' in name or 'out_transform' in name or 'attn1' in name or 'norm3' in name: 
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        train_loss, train_macc, train_oa, _, _ = \
            train_one_epoch(model, train_loader,
                            optimizer, scheduler, epoch, cfg, args)

        is_best = False
        if epoch % cfg.val_freq == 0:
            val_macc, val_oa, val_accs, val_cm = validate_fn(
                model, val_loader, cfg, args)
    ...
    # test the last epoch
    test_macc, test_oa, test_accs, test_cm = validate(model, test_loader, cfg, args)
    ...
    best_epoch, _ = load_checkpoint(model, pretrained_path=os.path.join(
        cfg.ckpt_dir, f'{cfg.run_name}_ckpt_best.pth'),if_stat=True)
    test_macc, test_oa, test_accs, test_cm = validate(model, test_loader, cfg, args)
    if writer is not None:
...
def train_one_epoch(model, train_loader, optimizer, scheduler, epoch, cfg, args):
    ...
    for idx, data in pbar:
        ...
        logits, loss = model.get_logits_loss(data, target, args) if not hasattr(model, 'module') else model.module.get_logits_loss(data, target)
        loss.backward()
        # optimize
...
@torch.no_grad()
def validate(model, val_loader, cfg, args):
