...
if __name__ == "__main__":
    ...
    parser.add_argument('--originmean', action='store_true', default=False, help='whether posembed use object mean')
    parser.add_argument('--num_view', type=int, default=6, help='') 
    parser.add_argument('--pos_cor', nargs='+', type=float, help='')
    parser.add_argument('--coef_3dgird', type=float, required=True, help='the coef of 3D grid size') 
    parser.add_argument('--coef_1dgird', type=int, required=True, help='th coef 1d adpater grid size')
    parser.add_argument('--attn1d_dim', type=int, required=True, help='th dim of 1d adpater attn')
    parser.add_argument('--maxmean_1d', action='store_true', default=False, help='whether the max or maxmean in 1d adapter')
    parser.add_argument('--scale_factor', type=float, default=1.0, help='')
    parser.add_argument('--coef_pro', type=float, required=True, help='')
    ...
    cfg.model.encoder_args.attn1d_dim = args.attn1d_dim
    ...
    if cfg.mode in ['resume', 'val', 'test']:
        ...
        generate_exp_directory(cfg, exp_name=cfg.formal_dirs, additional_id=os.environ.get('MASTER_PORT', None))
        cfg.wandb.tags = tags
